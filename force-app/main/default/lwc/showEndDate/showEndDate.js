...
export default class ShowEndDate extends LightningElement {
  @track
  columns = [
    {
      label: "SUN",
    ...
    for (let i = 0; i < 6; i++) {
      ...
      if (i === 0) {
        for (k = 1; k <= weekIndex; k++) {
          ...
          if (this.startDate) {
            style = this.day2SmallerThan1(prevDate, this.startDate)
              ? style
              : "background-color: #909699";
            disabled = this.day2SmallerThan1(prevDate, this.startDate)
              ? false
              : true;
          }
      ...
      for (let j = 0; j < 7 - k; j++) {
        ...
        if (this.startDate) {
          style = this.day2SmallerThan1(nextDate, this.startDate)
            ? style
            : "background-color: #909699";
          disabled = this.day2SmallerThan1(nextDate, this.startDate)
            ? false
            : true;
        }
        if (count > currentMonthDays) {
          ...
          if (this.startDate) {
            style = this.day2SmallerThan1(date, this.startDate)
              ? style
              : "background-color: #909699";
            disabled = this.day2SmallerThan1(date, this.startDate)
              ? false
              : true;
          }
          itemList.push({
  ...
  datePickHandler(evt) {
    ...
    if (this.startDate) {
      this.showDate = this.day2SmallerThan1(tmpDate, this.startDate)
