...
export default class Calendar2 extends LightningElement {
  ...
  toSaveEdit() {
    ...
    let inputStartDate = this.template.querySelector(
      ".inputStartDateEdit"
    ).value;
    let inputEndDateStatus = JSON.stringify(
    ...
    if (
      ...
      inputStartDateStatus !== "null" &&
      inputEndDateStatus !== "null" &&
      new Date(inputStartDate).getTime() < new Date(inputEndDate).getTime()
    ) {
      this.onProcess = true;
    ...
    for (let i = 0; i < 6; i++) {
      ...
      for (let j = 0; j < 7 - k; j++) {
        ...
        if (count > totalDay) {
          ...
          let date = new Date(this.yearNow, this.monthNow, count, 0, 0, 0, 0);
          style =
  ...
  arrangeEvents() {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      for (let j = 0; j < items.length; j++) {
        const ep = Object.assign({}, items[j], {
          classes: ["cv-item"],
          itemRow: 0
        });
